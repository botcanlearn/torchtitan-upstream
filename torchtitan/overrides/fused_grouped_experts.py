...
Importing this module registers the MinimalAsyncEP ``GroupedExperts`` override
and the ``torchtitan::silu_and_mul`` custom CUDA op it uses.
"""
...
def silu_and_mul_forward_kernel(
    ...
    """Compute ``silu(gate) * up`` with optional grouped-mm row offsets."""
...
class FusedGroupedExperts(GroupedExperts):
    """Grouped experts variant that uses the override SiLU-and-mul custom op.

    This is intended for ``MinimalAsyncEPTokenDispatcher`` only, where the
    receive buffer has capacity padding that grouped-mm offsets skip.
    """
    ...
    def __init__(self, config: Config):
        ...
        if not isinstance(self.token_dispatcher, MinimalAsyncEPTokenDispatcher):
            raise ValueError(
                "FusedGroupedExperts requires MinimalAsyncEPTokenDispatcher."
            )
...
@override(
    ...
    description="Use fused SiLU-and-mul activation for MinimalAsyncEP grouped experts.",
)
def fused_grouped_experts(
    cfg: GroupedExperts.Config,
) -> GroupedExperts.Config:
    if not isinstance(cfg.token_dispatcher, MinimalAsyncEPTokenDispatcher.Config):
        return cfg

    warn_once(
        logger,
        "fused_grouped_experts override active: GroupedExperts will use "
        "the torchtitan::silu_and_mul custom op for the SwiGLU "
        "activation.",
    )
