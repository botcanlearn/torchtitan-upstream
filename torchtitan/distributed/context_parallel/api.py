...
"""Context-parallel input and attention-metadata sharding APIs."""
...
_DEFAULT_INPUT_SEQ_DIMS = {"input": 0, "labels": 0, "positions": 0}
_BLOCK_MASK_QUERY_DIM = 2


def _get_cp_shard_dims(input_shardings: dict[str, SpmdType]) -> dict[str, int]:
...
def _create_context_parallel_load_balancer(
    ...
    if load_balancer_type == "headtail":
        return _HeadTailLoadBalancer(num_tokens, cp_world_size, cp_mesh.device_type)
...
def _shard_context_parallel_inputs(
    cp_mesh: DeviceMesh,
...
def prepare_context_parallel_input(
    ...
    """Shard model input tensors and return the load balancer for metadata.
    ...
    """
    shard_dims = (
        _get_cp_shard_dims(input_shardings)
        if input_shardings is not None
        else _DEFAULT_INPUT_SEQ_DIMS
    )
    named_tensors = {
        name: value
    ...
    sharded_inputs = _shard_context_parallel_inputs(
...
def cp_shard(
    ...
    inputs = _shard_context_parallel_inputs(
    ...
    if attention_masks is not None:
        ...
        masks = (
            [masks_to_shard]
            if isinstance(masks_to_shard, BlockMask)
            else list(masks_to_shard.values())
        )
        sharded_masks = cast(
            "tuple[BlockMask, ...]",
            _context_parallel_shard(
                mesh=cp_mesh,
                buffers=masks,
                seq_dims=(_BLOCK_MASK_QUERY_DIM,) * len(masks),
                load_balancer=load_balancer,
            ),
        )
        attention_masks = (
            sharded_masks[0]
            if isinstance(masks_to_shard, BlockMask)
            else dict(zip(masks_to_shard, sharded_masks))
        )
