...
class FusedQKVLinear(BaseQKVLinear):
    ...
    def _merge_qkv_on_load(module, state_dict, prefix, *args) -> None:
        ...
        wq_key, wk_key, wv_key = f"{prefix}wq.weight", f"{prefix}wk.weight", f"{prefix}wv.weight"

        if wq_key in state_dict and wk_key in state_dict and wv_key in state_dict:
            wq, wk, wv = state_dict.pop(wq_key), state_dict.pop(wk_key), state_dict.pop(wv_key)
            n_kv, in_f = wk.shape[0] // hd, wq.shape[1]
            q = wq.reshape(n_kv, hpk, hd, in_f)
            k = wk.reshape(n_kv, 1, hd, in_f)
            v = wv.reshape(n_kv, 1, hd, in_f)
            state_dict[f"{prefix}wqkv.weight"] = torch.cat([q, k, v], dim=1).reshape(-1, in_f)

        bq_key, bk_key, bv_key = f"{prefix}wq.bias", f"{prefix}wk.bias", f"{prefix}wv.bias"
        if bq_key in state_dict and bk_key in state_dict and bv_key in state_dict:
            bq, bk, bv = state_dict.pop(bq_key), state_dict.pop(bk_key), state_dict.pop(bv_key)
            n_kv = bk.shape[0] // hd
            q_b = bq.reshape(n_kv, hpk, hd)
            k_b = bk.reshape(n_kv, 1, hd)
            v_b = bv.reshape(n_kv, 1, hd)
            state_dict[f"{prefix}wqkv.bias"] = torch.cat([q_b, k_b, v_b], dim=1).reshape(-1)
